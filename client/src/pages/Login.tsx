...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const serverMessage = error.response?.data?.message || "Login gagal. Email atau password salah.";
    ...
    <div className="min-h-screen bg-[#0F172A] text-slate-100 flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-400">Masuk untuk mengelola dashboard inventory UMKM Anda</p>
        </div>

        <Card className="border-slate-800 bg-[#1E293B]/80 backdrop-blur-md text-slate-100 shadow-xl shadow-cyan-950/20">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl text-center text-white">Login ke Akun Anda</CardTitle>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <CardFooter className="flex flex-col gap-4">
              ...
              <div className="text-sm text-center text-slate-400 mt-2">
                Belum memiliki akun?{" "}
                <Link href="/register" className="text-cyan-400 hover:underline hover:text-cyan-300 font-medium transition-colors">
                  Daftar di sini
                </Link>
              </div>
            </CardFooter>
